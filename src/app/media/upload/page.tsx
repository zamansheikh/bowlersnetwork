...
export default function MediaUploadPage() {
    ...
    const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Initialize upload state without waiting for duration
        setUploadState({
            step: 'idle',
            error: null,
            uploadProgress: 0,
        ...
        // Get video duration asynchronously (non-blocking)
        try {
            ...
            console.warn('Failed to get video duration, will use 0:', error);
            // Continue without duration, we'll use 0 as default during upload
        }
    ...
    const handleVideoUpload = async () => {
        ...
        try {
            ...
            // Step 2: Upload video file to presigned URL with progress tracking
            console.log('[Upload] Uploading video to presigned URL');
            await uploadWithProgress(uploadState.videoFile, presigned_url);
            ...
            const videoUid = responseData.uid;
            ...
            // Reset form and redirect to video details page after a short delay to show success state
            setTimeout(() => {
                setTitle('');
                setDescription('');
                setIsTitleValid(false);
                ...
                if (videoUid) {
                    router.push(`/media/videos/${videoUid}`);
                } else {
                    router.push('/media?tab=videos');
                }
            }, 1500);
        } catch (error: any) {
