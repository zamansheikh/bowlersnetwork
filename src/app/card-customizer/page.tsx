...
export default function CardCustomizerPage() {
    ...
    const { uploadFile, isUploading } = useCloudUpload();
    ...
    const handleGenerateCard = async () => {
        if (!selectedDesign) { setError('Please select a card design'); return; }
        if (selectedBrands.length !== 4) { setError('Please select exactly 4 brands'); return; }
        if (!displayPictureUrl.trim()) { setError('Please upload a display picture'); return; }
        if (!displayName.trim()) { setError('Please provide a display name'); return; }
        if (displayName.length > 18) { setError('Display name must be 18 characters or less'); return; }
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">Design your personalized trading card</p>
                                </div>
                            </div>
                            {cardHtml && (
                                <button
                                    ...
                                    disabled={loading}
                                    className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg shadow-md hover:shadow-lg transition-all duration-200 flex items-center gap-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {loading ? (
                        ...
                        <div className="lg:col-span-1 space-y-2">
                                    ...
                                    <div className="grid grid-cols-4 gap-2 max-h-[200px] overflow-y-auto">
                                        {brands.map((brand) => (
                                            <button
                                                ...
                                                className={`relative aspect-square rounded-lg border-2 transition-all overflow-hidden ${selectedBrands.includes(brand.brand_id)
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3">
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">Upload your image (max 5MB)</p>
                                    </div>
                                </div>
                                        ...
                                        >
                                            {isUploading ? (
                                                ...
                                                <>
                                                    ...
                                                    <span className="text-xs font-medium text-gray-700 text-center">Click to upload</span>
                                                </>
                                            )}
                                        </label>
                                    </div>
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3">
                                ...
                                <input
                                    ...
                                    value={displayName}
                                    onChange={(e) => {
                                        if (e.target.value.length <= 18) setDisplayName(e.target.value);
                                    }}
                                    maxLength={18}
                                    placeholder="Enter your name"
                                    className="w-full px-3 py-2 border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition text-sm"
                                />
                            </div>
                            ...
                            <button
                                ...
                                className="w-full px-6 py-3 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 disabled:text-gray-500 text-white font-bold rounded-xl shadow-md hover:shadow-lg hover:scale-[1.02] disabled:shadow-none disabled:hover:scale-100 transition-all duration-200 flex items-center justify-center gap-2 text-base mt-2"
                            >
                                {loading ? (
                                    <>
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sticky top-20">
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">Your card will appear here after generating</p>
                                    </div>
                                </div>
                                ...
                                <div className="border-2 border-gray-100 rounded-xl bg-gray-50/50 p-4 min-h-[500px] flex items-center justify-center">
                                    {loading ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center text-center max-w-xs">
                                            ...
                                            <h4 className="text-base font-semibold text-gray-900 mb-1">Ready to Customize</h4>
                                            <p className="text-sm text-gray-500">
                                                Fill in all the details on the left and click <strong>Generate Card</strong> to preview your trading card.
                                            </p>
                                            <ul className="mt-4 text-left text-xs text-gray-400 space-y-1">
                                                <li className={`flex items-center gap-1 ${selectedDesign ? 'text-green-600' : ''}`}>
                                                    {selectedDesign ? <Check className="w-3 h-3" /> : <span className="w-3 h-3 inline-block rounded-full border border-gray-300" />}
                                                    Select a card design
                                                </li>
                                                <li className={`flex items-center gap-1 ${selectedBrands.length === 4 ? 'text-green-600' : ''}`}>
                                                    {selectedBrands.length === 4 ? <Check className="w-3 h-3" /> : <span className="w-3 h-3 inline-block rounded-full border border-gray-300" />}
                                                    Choose 4 brands ({selectedBrands.length}/4)
                                                </li>
                                                <li className={`flex items-center gap-1 ${imageUploaded ? 'text-green-600' : ''}`}>
                                                    {imageUploaded ? <Check className="w-3 h-3" /> : <span className="w-3 h-3 inline-block rounded-full border border-gray-300" />}
                                                    Upload a profile photo
                                                </li>
                                                <li className={`flex items-center gap-1 ${displayName ? 'text-green-600' : ''}`}>
                                                    {displayName ? <Check className="w-3 h-3" /> : <span className="w-3 h-3 inline-block rounded-full border border-gray-300" />}
                                ...
                                {cardHtml && (
                                    <div className="mt-4 flex gap-3">
                                        <button
                                            onClick={handleGenerateCard}
                                            disabled={loading}
                                            className="flex-1 px-4 py-2 border-2 border-gray-200 hover:border-gray-300 text-gray-700 font-semibold rounded-lg text-sm transition flex items-center justify-center gap-2"
                                        >
                                        ...
                                        <button
                                            onClick={handleSaveCard}
                                            disabled={loading}
                                            className="flex-1 px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg text-sm transition flex items-center justify-center gap-2 disabled:opacity-50"
                                        >
